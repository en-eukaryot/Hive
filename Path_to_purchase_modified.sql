...
-- Date for latest device matching
SET device_matched_date = '2016-10-25';
...
DROP TABLE IF EXISTS cluo.all_buyers_id_test;

CREATE TABLE cluo.all_buyers_id_test AS
...
JOIN bi_data.uid_matching_full_lt ma
    ON ev.uid = ma.uid
        AND ma.day = ${hiveconf:device_matched_date}
...
WHERE
    ...
    AND ev.partnerid = ${hiveconf:partnerid}
...
-- DROP TABLE IF EXISTS cluo.all_buyers_id_full_test;
CREATE TABLE cluo.all_buyers_id_full_test AS
SELECT *
    FROM
    (
    SELECT
    ...
    SELECT
        ...
        FROM cluo.all_buyers_id_test buyers
        JOIN
            (
            SELECT
                uid
            ...
            WHERE day = ${hiveconf:device_matched_date}
            ) matches
...
DROP TABLE IF EXISTS cluo.all_buyers_events_test;
CREATE TABLE cluo.all_buyers_events_test AS
SELECT
	ev.`timestamp`
	,buyers.partner_id
	-- ,buyers.country
...
FROM cluo.all_buyers_id_full_test buyers
